...
<script setup>
  ...
  const contact = computed(() => store.state.contacts.find(e => e._id == route.params.id))

  const formRef = ref()
  const phone_number_error = ref()
  const email_error = ref()
  const fullname = ref(contact.value.fullname)
  ...
  const submitForm = (formEl) => {
    ...
    formEl.validate(async (valid) => {
      ...
      if (valid) {
        fullname.value = formFields.fullname
        store.dispatch('EditContact', formFields)
      } else {
